...
def run_block(win, outlet_handle, n, length, practice=False, block_num=1):
    ...
    # NEW: difficulty labels (block-level)
    label = "Baseline (0-back)" if n == 0 else f"{n}-back"
    log_marker(f"{block_type}_block_{block_num}_n_{n}")
    log_marker(f"{block_type}_block_{block_num}_difficulty_{label}")

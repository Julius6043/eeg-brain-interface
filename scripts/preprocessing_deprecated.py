...
def eeg_stream_to_raw(
    ...
    if med_abs > 1e-3:
        print(
            f"[INFO] Data median abs={med_abs:.1f} (looks like µV) -> scaling to Volts (x1e-6)"
        )
        data *= 1e-6
    else:
...
def save_raw_segment(
    raw: mne.io.Raw, out: Path, seconds: float = 10.0, uV: float = 100.0
):
...
def export_ml(raw: mne.io.Raw, out_npz: Path, win_s=2.0, hop_s=1.0):
    ...
    X_raw = (
        np.stack(X_raw, axis=0)
        if X_raw
        else np.empty((0, data.shape[0], win), dtype=np.float32)
    )
    ...
    np.savez(
        ...
        win_samples=np.array(win, dtype=np.int32),
        hop_samples=np.array(hop, dtype=np.int32),
        ch_names=np.array(raw.ch_names, dtype=object),
    )
    print(
        f"[OK] Saved ML tensors/features -> {out_npz} | windows={len(starts)} shape_raw={X_raw.shape} shape_feat={X_feat.shape}"
    )
...
def main():
    ap = argparse.ArgumentParser(
        description="EEG preprocessing for BCI/ML (XDF → plots/FIF/features)."
    )
    ...
    ap.add_argument(
        ...
        nargs="+",
        default=None,
        help="Channel names to keep (e.g., EEG1 EEG2 ...). Defaults to first 8.",
    )
    ap.add_argument(
        "--mains", type=int, default=50, help="Mains frequency (50 EU / 60 US)."
    )
    ...
    ap.add_argument(
        ...
        default=[1.0, 40.0],
        metavar=("L_HZ", "H_HZ"),
        help="Band-pass [low high] Hz.",
    )
    ap.add_argument("--no-bp", action="store_true", help="Skip band-pass filtering.")
    ap.add_argument(
        "--reref", action="store_true", help="Average re-reference after filters."
    )
    ap.add_argument(
        "--ica",
        action="store_true",
        help="Run ICA and remove EOG components (best-effort).",
    )
    ap.add_argument(
        "--win",
        type=float,
        default=2.0,
        help="Sliding window length (s) for ML export.",
    )
    ap.add_argument(
        "--hop", type=float, default=1.0, help="Sliding window hop (s) for ML export."
    )
    ap.add_argument(
        "--save-ml",
        action="store_true",
        help="Export ML windows and bandpower features (npz).",
    )
    ap.add_argument(
        "--uV",
        type=float,
        default=100.0,
        help="Plot vertical scale per channel in microvolts.",
    )
    args = ap.parse_args()
    ...
    raw = eeg_stream_to_raw(
        eeg_stream, channels_keep=args.channels, montage="standard_1020"
    )
    print(raw)
    print(
        f"[INFO] Duration ≈ {raw.n_times / raw.info['sfreq']:.1f} s; sfreq={raw.info['sfreq']} Hz"
    )
    ...
    if not args.no_notch:
        ...
        save_raw_segment(
            raw_proc, outdir / "raw_after_notch.png", seconds=10.0, uV=args.uV
        )
        save_psd(
            raw_proc, outdir / "psd_after_notch.png", fmin=1, fmax=120, ylim=(-40, 20)
        )
    else:
    ...
    if not args.no_bp:
        ...
        save_raw_segment(
            raw_proc, outdir / "raw_after_bandpass.png", seconds=10.0, uV=args.uV
        )
        save_psd(
            raw_proc,
            outdir / "psd_after_bandpass.png",
            fmin=1,
            fmax=120,
            ylim=(-40, 20),
        )
    ...
    if args.reref:
        ...
        save_raw_segment(
            raw_proc, outdir / "raw_after_reref.png", seconds=10.0, uV=args.uV
        )
        save_psd(
            raw_proc, outdir / "psd_after_reref.png", fmin=1, fmax=120, ylim=(-40, 20)
        )
        print(
            "[INFO] custom_ref_applied =",
            raw_proc.info.get("custom_ref_applied", False),
        )
    else:
    ...
    if args.ica:
        try:
            ...
            raw_for_ica = raw_proc.copy().filter(
                l_freq=1.0,
                h_freq=None,
                picks="eeg",
                method="fir",
            ...
            ica = ICA(
                n_components=None, random_state=97, method="fastica", max_iter="auto"
            )
            ica.fit(raw_for_ica)
            ...
            try:
                eog_epochs = create_eog_epochs(
                    raw_proc, ch_name=None, reject_by_annotation=True
                )
            ...
            axes[0].set_title("Before ICA (10 s)")
            axes[1].set_title("After ICA (10 s)")
            for ax in axes:
                ax.set_xlim(0, nsec)
            ...
            fig.tight_layout()
            fig.savefig(outdir / "ica_compare_timeseries.png", dpi=150)
            plt.close(fig)
            ...
            ax.set_xlabel("Frequency (Hz)")
            ax.set_ylabel("Power (dB/Hz)")
            ax.set_title("PSD (mean across channels)")
            ax.grid(True, alpha=0.3)
            ax.legend()
            fig.tight_layout()
            fig.savefig(outdir / "ica_compare_psd.png", dpi=150)
            plt.close(fig)
    ...
    if args.save_ml:
        export_ml(
            raw_proc,
            outdir / "ml_windows_bandpower.npz",
            win_s=args.win,
            hop_s=args.hop,
        )

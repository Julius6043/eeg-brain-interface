import csv
import re # regex for redundancy files
import random
...
BREAK_DURATION     = 60    # seconds, adjustable break between main blocks

FIXATION_DUR       = 2.0
STIM_DUR           = 1.0
FEEDBACK_DUR       = .5     # only relevant for practice
...
def main():
    ...
    # 1) Window & LSL Outlet
    info = StreamInfo("nBack_markers", "markers", 1, 0, 'string', LSL_ID)
    outlet = StreamOutlet(info)
    outlet.push_sample(["streamInitialized"])
    ...
    welcome = visual.TextStim(
        win,
        text="Welcome to N-back.\n\nFirst: 2-minute baseline and then 0-back baseline trial.\nPress any key to start.",
        color="white", wrapWidth=1.2, height=0.06
    )
    welcome.draw(); win.flip()
    event.waitKeys(); check_for_quit(win)
    
    # 2-minute still baseline
    
    instr = visual.TextStim(
        win,
        text=(
            "Stay as still as possible,\n"
            "and avoid movements or eye blinks/movements during this period.\n\n"
    ...
    baseline_timer = core.CountdownTimer(120)  # 120 seconds
    countdown_txt  = visual.TextStim(
        win,
        text="Seconds to go",
        color="white",
        wrapWidth=1.2,
    ...
    while baseline_timer.getTime() > 0:
        m, s = divmod(int(baseline_timer.getTime()), 60)
        countdown_txt.text = f"Baseline recording\nTime left: {m:02d}:{s:02d}"
        countdown_txt.draw()
        win.flip()
        core.wait(1.0)
        check_for_quit(win)
